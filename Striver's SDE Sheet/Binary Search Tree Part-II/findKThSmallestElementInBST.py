# QUESTION: Find K-th smallest element in BST
# Given the root node of a binary search tree (BST) and an integer k. Return the kth smallest and largest value (1-indexed) of all values of the nodes in the tree. Return the 1st i.
#
# Examples:
# Example 1:
# Input: root = [3, 1, 4, null, 2], k = 1
# Output: [1, 4]
# Explanation:
# The 1st smallest value in the given BST is 1.
# The 1st largest value in the given BST is 4.
#
# Example 2:
# Input: root = [5, 3, 6, 2, null, null, null, 1], k = 3
# Output: [3, 3]
# Explanation:
# The 3rd smallest value in the given BST is 3.
# The 3rd largest value in the given BST is 3.


"""
#Brute Force:
1.
TC -> O(), SC -> O()

#Better Approach:
1.
TC -> O(), SC -> O()

#Optimal Approach:
1.
TC -> O(), SC -> O()

#KEY INSIGHT:
-
"""


class Solution:
    def find_k_th_smallest_element_in_bst_brute(self) -> None:
        pass

    def find_k_th_smallest_element_in_bst_better(self) -> None:
        pass

    def find_k_th_smallest_element_in_bst_optimal(self) -> None:
        pass


if __name__ == "__main__":
    sol = Solution()
