...
class Solution:   
  ...
  def findSolution(self, arr: List[int], k: int):
    left, right, maxLen, sum = 0, 0, 0, arr[0]
    n = len(arr)
    while (right < n):
      # left should not go over right and sum should be less than k
      ...
      # Increment right First then Check If it is Still Less than N to Add to Sum
      right += 1
      if (right < n):
        sum += arr[right]
...
if __name__ == "__main__":
  ...
  k = 7
  resp = Solution().findSolution2(arr, k)
  print(resp)
...
#Optimal Solution (Two Pointer With Sliding Window Approach):
1. Initialize Pointers i, j = 0 with Sum = a[0] and check sum == k
2. While j < n Check If incrementing J goes over sum or not.
3. If It does increment i first to reduce window size decresing sum with it
4. Check If Current Sum is Equal to K and find out max of Length
5. Now Increment J and check whether it is still less than n then add to Sum
TC -> O(2n), SC -> O(1)
"""