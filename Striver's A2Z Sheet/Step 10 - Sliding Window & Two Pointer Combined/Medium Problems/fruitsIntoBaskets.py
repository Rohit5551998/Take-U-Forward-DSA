...
3. If size of set is less than 2 compute length and perform max operation else break out of inner loop
4. Return maxLen
...
3. Run another while loop till size of mpp is greater than 2 and decrement mpp[arr[left]] by 1 for each arr[left], if it is 0 
remove it from mpp and increment left at end of while.
4. If size of mpp is <= 2 compute len and then perform max operation, increment r and return maxAns at end of loop
TC -> O(2N), SC -> O(1)

Optimal Approach 2 (Constant Max Window)
1. Initialize maxLen, left, right to 0, mpp to {} and run a loop till right < len(array)
2. If arr[right] is not in mpp, set mpp[arr[right]] = 0 and increment it by 1
3. If size of mpp is greater than 2 and decrement mpp[arr[left]] by 1 for each arr[left], if it is 0 remove it from mpp and 
increment left at end of while.
4. Else if size of mpp is <= 2 compute len and then perform max operation, increment r and return maxAns at end of loop
TC -> O(N), SC -> O(1)
"""